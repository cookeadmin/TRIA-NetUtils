...
# perl fastq_barcode_splitter_cascade.pl -i ~/workspace/GBS_data-08-10-2013/HI.1405.008.GQ03122013-5_R1.fastq -b ~/workspace/GBS_data-08-10-2013/GBS_barcodes-2013-10-09.csv -o ~/workspace/GBS_data-08-10-2013/SPLIT_FASTQ_OUTFILES
...
foreach my $barcode_seq_length (sort {$b <=> $a} keys %barcode_data){
	...
	my $barcodes_lengths_dir = join('/', $output_dir, "FASTQ_BARCODE_LENGTH_DIR");
	...
	my $barcode_splitter_file = join('/', $output_dir, "fastq_barcodes_length_$barcode_seq_length" . ".txt");
	...
	foreach my $fastq_barcode_data (@{$barcode_data{$barcode_seq_length}}){
		...
		my $project_leader_dir = join('/', $output_dir, "PROJECT_LEADER_DIR");
