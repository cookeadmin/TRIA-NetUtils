...
print OUTFILE join("\t", "name","ec_code","x_coord","y_coord","width","height","html_source") . "\n";
foreach my $data (@xml_formated_data){

      if($data =~ m/<area shape="rect" coords="(\d+),(\d+),(\d+),(\d+)" href="(.+)" title="(.+) \((.+)\)">/){
	    my ($x_coord,$y_coord,$width,$hieght,$html_source,$name, $ec_code) = ($1,$2,$3,$4,$5,$6,$7);
	    print OUTFILE join("\t", $name,$ec_code,$x_coord,$y_coord,$width,$hieght,$html_source) . "\n";
      }
