...
my $fasta_filename = fileparse($fasta_infile, qr/\.fasta/);

my $fasta_outfile = join('/', $output_dir, $fasta_filename . ".fastq");
...
while(my $seq_entry = $seqio->next_seq) {
	...
	print OUTFILE join("", "\@", $seq_id) . "\n";
	print OUTFILE $sequence . "\n";
	print OUTFILE join("", "+", join(" ", $seq_id, $seq_desc, "length=$sequence_length")) . "\n";
	print OUTFILE ('J' x $sequence_length) . "\n";
}
